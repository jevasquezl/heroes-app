...
import { useHeroSummary } from "../hoocks/useHeroSummary"
...
export const HeroStats = () => {
    ...
    if (!summary) return (<h3>Sin Registros</h3>)

    const favoritePorcent = (favoriteCount / summary.totalHeroes * 100).toFixed(2);
    ...
    return (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            ...
            <HeroStatCard title="Favorites" icon={<Heart className="h-4 w-4 text-muted-foreground" />}>
                ...
                <p className="text-xs text-muted-foreground">{favoritePorcent} % of total</p>
            </HeroStatCard>
